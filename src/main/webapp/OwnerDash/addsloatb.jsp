...
<body>
	...
	<div class="container">
		...
		%>
		<form action="../addSloatb" method="post" class="slots">
			<h2>
				<span>A</span>dd <span>S</span>lots
			</h2>

			<input type="hidden" name="boxOwnerId" class="box"
				value="<%=owner.getId()%>" required> <input type="hidden"
				name="boxId" class="box" value="<%=bid%>" required>
			<h4>Date</h4>
			<input type="date" name="Dateb" class="box"
				min="<%=LocalDate.now()%>" max="<%=LocalDate.now()%>" required>
			<h4>Starting Time</h4>
			<input type="time" name="startTimeb" class="box" required>
			<h4>Ending Time</h4>
			<input type="time" name="endTimeb" class="box" required> <input
	...
	<div>
		<table>
			...
			<tbody>
				...
				List<Sloatb> stb = dao.getSlotById(bid);
				int index = 1;
				for (Sloatb bs : stb) {
				%>
				<tr>
					<td><%=index%></td>
					<td><%=bs.getDate()%></td>
					<td><%=bs.getTimeFrom()%> to <%=bs.getTimeTo()%></td>
				</tr>
				<%
				index++;
				}
